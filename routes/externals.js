...
var REPORTS_DIR = "./public/reports"
 	...
 	form.parse(req, function(err, fields, files) {
		if(err){
			data.status = "false";
	 		...
	 		var jData = JSON.stringify(new Buffer(jData).toString('base64'));
	 		res.end(jData);
	 		return;
		}
		...
		if(!files.report_file){
			data.status = "false";
	 		data.res_code = "REPORT_FILE_NOT_FOUND";
			...
			if(data.res_code == "LAB_INSERTED" || data.res_code == "LAB_UPDATED"){
				try{
					...
					fs.createReadStream(file.path)
						.pipe(fs.createWriteStream(REPORTS_DIR + "/" + fileName ) );
				}
				catch(e){
					data.status = "false";
