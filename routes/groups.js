		...
		var query = 
			...
			LEFT JOIN (
				SELECT
					*
				FROM 
					users_class_groups us
				WHERE us.users_id = ?
			) AS u ON u.class_group_id = c.id`
		;
...
router.get('/getGroupsByUser', function(req, res) {
	var buf = Buffer.from(req.query.data, 'base64');
	var params = JSON.parse(buf);
	...
	connection.connect(function(err) {
		...
		var query = 
			`SELECT
				c.id as group_id,
				u.id as user_group_id, 
				c.\`name\`					
			FROM
				class_group c
			LEFT JOIN users_class_groups u ON u.class_group_id = c.id
			WHERE 
				u.users_id = ?` 
		;
